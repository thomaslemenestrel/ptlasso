...
def _feature_color_map(fit):
    ...
    cmap = plt.cm.get_cmap("tab20", max(len(active), 1))
    return {j: cmap(i) for i, j in enumerate(active)}
...
def plot_cv(fit, ax=None, plot_alphahat=True, column="single", save=None):
    """Plot the cross-validation curve for a PretrainedLassoCV.
    ...
    ax.fill_between(alphas, cv_mean - cv_se, cv_mean + cv_se,
                    color=COLORS["pretrain"], alpha=0.15, linewidth=0)
    ax.plot(alphas, cv_mean, color=COLORS["pretrain"],
            marker="o", markersize=5, linewidth=2, label="Pretrain", zorder=3)

    # Individual and overall baselines
    ax.axhline(fit.cv_results_individual_, color=COLORS["individual"],
               linestyle="--", linewidth=1.8)
    ax.axhline(fit.cv_results_overall_,    color=COLORS["overall"],
               linestyle="--", linewidth=1.8)

    # Selected alpha
    if plot_alphahat:
        ax.axvline(fit.alpha_, color="#555555", linestyle=":",
                   linewidth=1.5, label=f"$\\hat{{\\alpha}}={fit.alpha_}$")
    ...
    xmax  = max(alphas)
    ax.text(xmax + 0.01, fit.cv_results_individual_,
            f"Individual  $|\\hat{{S}}|={n_ind}$",
            va="center", fontsize=8, color=COLORS["individual"], clip_on=False)
    ax.text(xmax + 0.01, fit.cv_results_overall_,
            f"Overall  $|\\hat{{S}}|={n_ov}$",
            va="center", fontsize=8, color=COLORS["overall"], clip_on=False)
...
def _draw_paths(ax, state, title, color, feature_colors, support, labels, xlim):
    ...
    for j in sorted(final, key=lambda j: abs(betas[-1, j]), reverse=True):
        ...
        if all(abs(y_end - y) > 0.04 * y_range for y in placed):
            ax.text(x_end + 0.05 * (xlim[1] - xlim[0]), y_end, name,
                    fontsize=6.5, va="center", color=c, clip_on=False)
            placed.append(y_end)
    ...
    ax.set_title(f"{title}  $(|\\hat{{S}}|={len(final)})$",
                 fontsize=9, pad=5, color=color, fontweight="bold")
    ax.tick_params(labelsize=7)


def plot_paths(fit, column="double", save=None):
    """Plot regularisation paths for all sub-models in a PretrainedLasso.

    Layout: row 0 = overall (full width), row 1 = pretrain, row 2 = individual.
    ...
    overall_sup = set(np.where(
        np.reshape(fit.overall_coef_, (fit.n_features_in_, -1)).any(axis=1)
    )[0])
    ...
    _draw_paths(fig.add_subplot(gs[0, :]), fit.overall_model_,
                "Overall", COLORS["overall"], feature_colors, overall_sup, labels, xlim)
    ...
    for col, g in enumerate(fit.groups_):
        ...
        _draw_paths(fig.add_subplot(gs[1, col]), fit.pretrain_models_[g],
                    lbl, COLORS["pretrain"], feature_colors, pre_sup, labels, xlim)
        _draw_paths(fig.add_subplot(gs[2, col]), fit.individual_models_[g],
                    lbl, COLORS["individual"], feature_colors, ind_sup, labels, xlim)
    ...
    for row, (text, color) in enumerate([
        ...
        fig.text(0.01, 1 - (row + 0.5) / 3, text,
                 va="center", ha="center", fontsize=10, fontweight="bold",
                 color=color, rotation=90, transform=fig.transFigure)
